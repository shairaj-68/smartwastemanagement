...
const Sidebar = ({ role = 'citizen', onLogout }) => {
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full bg-slate-900 border-r border-slate-800 transition-all duration-300 ease-in-out shadow-xl">
      {/* Brand Header */}
        ...
        <div className="flex items-center gap-3 overflow-hidden">
          ...
          {!isCollapsed && (
            <div className="flex flex-col">
              ...
              <span className="text-[10px] text-emerald-400 font-bold tracking-[0.2em] uppercase leading-none mt-1">
                Enterprise
              </span>
            </div>
          )}
        ...
        {menuGroups.map((group) => {
          ...
          return (
            <div key={group.title} className="space-y-2">
              {!isCollapsed && (
                <h3 className="text-[11px] font-bold text-slate-500 px-3 tracking-[0.15em] mb-3">
                  {group.title}
                </h3>
              )}
                ...
                {visibleItems.map((item) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {isActive && (
                        <div className="absolute right-0 top-1/2 -translate-y-1/2 w-1 h-6 bg-emerald-500 rounded-l-full shadow-[0_0_12px_rgba(16,185,129,1)]" />
                      )}
                    </button>
                  );
                })}
  ...
  return (
    <>
      ...
      <aside className={cn(
        "hidden md:block transition-all duration-500 ease-in-out h-screen sticky top-0",
      ...
      <aside className={cn(
        "md:hidden fixed inset-y-0 left-0 bg-slate-900 z-50 transition-transform duration-500 cubic-bezier(0.4, 0, 0.2, 1) w-72 shadow-2xl",
